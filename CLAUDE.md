...
**Deployment: `Dockerfile` targets Cloud Run.** `src/app.py` exposes a top-level `server = app.server` (the underlying Flask instance) — gunicorn ≥26 no longer parses attribute-access import specs like `app:app.server`, so the Dockerfile's `CMD` runs `gunicorn ... app:server` instead. The image installs `requirements.txt` (not `requirements-dev.txt` — no `pytest` in the runtime image) plus `gunicorn`, and copies only `src/` and `data/raw/merged_df.csv`; it binds `$PORT` (default 8080) per Cloud Run convention. See `README.md` for build/run commands. `docs/DESIGN.md` §8 lists the platform comparison that led here (Cloud Run picked because GCP was already set up for this project) — that section still frames it as "2차, 참고용" and hasn't been rewritten to reflect the decision.
